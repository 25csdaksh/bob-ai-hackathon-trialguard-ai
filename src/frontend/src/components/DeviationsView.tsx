...
export const DeviationsView: React.FC<DeviationsViewProps> = ({ deviations, onSelectDeviation }) => {
  ...
  const getSeverityBadge = (sev: string) => {
    switch (sev) {
      case 'Major':
        return <span className="px-2.5 py-0.5 bg-rose-500/10 border border-rose-500/30 text-rose-400 font-bold text-xs rounded-full">Major</span>;
      case 'Minor':
        return <span className="px-2.5 py-0.5 bg-amber-500/10 border border-amber-500/30 text-amber-400 font-bold text-xs rounded-full">Minor</span>;
      default:
        return <span className="px-2.5 py-0.5 bg-blue-500/10 border border-blue-500/30 text-blue-400 font-bold text-xs rounded-full">Administrative</span>;
    }
  };
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400">Recorded rule engine violations across all sites ({deviations.length} total)</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          ...
          <select
            ...
            className="px-3 py-2 bg-dark-800 border border-slate-700 rounded-xl text-xs text-white focus:outline-none focus:border-blue-500"
          >
            ...
            <tbody className="divide-y divide-slate-800 text-slate-200">
              {filteredDeviations.length === 0 ? (
                ...
                filteredDeviations.map((dev) => (
                  ...
                  >
                    <td className="p-4 font-mono font-bold text-blue-400">{dev.deviation_id}</td>
                    <td className="p-4 font-medium text-white">{dev.patient_id}</td>
                    <td className="p-4">{dev.site_id}</td>
                    <td className="p-4 font-medium text-slate-200">{dev.deviation_type}</td>
                    <td className="p-4">{getSeverityBadge(dev.severity)}</td>
                    <td className="p-4 text-slate-400">{dev.date}</td>
                    <td className="p-4 text-right">
                      <button className="px-3 py-1.5 bg-slate-800 group-hover:bg-blue-600 text-slate-300 group-hover:text-white rounded-lg text-xs font-semibold transition-all inline-flex items-center space-x-1">
                        <span>Inspect</span>
